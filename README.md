...
브라우저에서 이미지를 보정하고, `Original`, `2x 업스케일`, `4x 업스케일` 결과를 비교한 뒤 PNG/JPG로 저장하는 V1 프로젝트다. `2x 업스케일`은 원본을 먼저 보정한 뒤 업스케일 전용 후처리를 거친 결과를 만들고, `4x 업스케일`은 데스크톱 Chromium에서만 제공되는 모델 기반 초해상도 복원 경로를 사용한다. UI는 화이트톤 배경 위에 블루 포인트, 블루 섹션 제목, 한 줄 히어로 타이틀, 더 컴팩트한 지원 브라우저 배지 기준으로 정리했다.

## 배포 주소
- 프로덕션: https://hg-image-enhancer.vercel.app
- 최신 배포는 Vercel 프로덕션 별칭에 연결된다.
...
│  ├─ main.tsx                # React 진입점
│  ├─ styles.css              # 화이트톤 배경 + 블루 포인트/섹션 제목/히어로 타이틀/지원 배지 스타일
│  ├─ types.ts                # 공용 타입 정의
│  ├─ lib/
...
증상:
- 24MP를 넘는 이미지 업로드 시 출력 방식 선택 패널이 표시되거나 결과 크기가 자동으로 clamp됨
